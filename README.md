...
### 步驟二：準備字型檔案為確保中文浮水印能正常顯示，請準備一個支援中文的字型檔 (.ttf 或 .otf)。預設路徑為 Windows 的 C:\Windows\Fonts\mingliu.ttc (新細明體)。如果您在其他作業系統或使用不同的字型，請修改程式中的 DEFAULT_FONT_PATH 變數，或是在執行時使用 --font 參數指定。💡 使用方法基本命令格式Bashpython main.py --input <輸入檔案路徑> --output <輸出檔案路徑> [其他參數]
